...
hero call (Cartesia Devansh, self-check OK, failures 0:15/0:48) · money-shot page /shot (click-to-seek verified) · normalize+signals over **46 calls** (44 SpokenWOZ + hero + Bolna, Batch 2) · Gemini judge harness (smoke + cache) · Bolna agent live, **Cartesia configured inside its synthesizer** (provider=cartesia/Devansh) + 1 ingested execution (246cd9f3…, predates that Cartesia config) · runtime keys verified: BOLNA + GEMINI (Cartesia needs NO separate key — it runs inside Bolna) · 36 training notebooks.
